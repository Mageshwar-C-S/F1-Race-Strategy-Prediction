...
@st.cache_resource
def position_analysis(_lap_in, _out, num_of_laps=1, line_chart={}):
    # Initialize an empty list to store rows
    rows = []
    
    _lap = _lap_in.detach().clone().numpy()  # Assuming _lap_in is a torch tensor
    _o = _out.detach().clone().numpy()  # Assuming out is a torch tensor
    _name, _loc, _country = circuit_info(np.argmax(_lap[:130]))  # Make sure circuit_info is defined

    for i in range(20):
        _d_idx = driver_unbed_idx(driver_unbed(_lap[131 + i * 196 : 131 + i * 196 + 130]))  # Make sure driver_unbed_idx is defined
        _num, _code, _fn, _ln, _, _ = driver_info(_d_idx)  # Make sure driver_info is defined
        ...
        if (_pos == 21):
            _retired = True
        _status = stat_unbed(_o[i*60 + 22: i*60 + 28], _retired)  # Make sure stat_unbed is defined
        if (_d_idx == 853):
            _status = 'Spun off'
        if (_retired):
            _time = 'NA'
        else:
            _time = lt_unbed(_o[i*60 + 28:])  # Make sure lt_unbed is defined
            _time = f'{_time:.1f}'
...
@st.cache_data
def out_to_in(_in_, _out_, random=False, num_of_laps=50, randomness=20):
  _ret = _in_.detach().clone().numpy()
  _o = _out_.detach().clone().numpy()
  _o = _o.reshape([1200])
