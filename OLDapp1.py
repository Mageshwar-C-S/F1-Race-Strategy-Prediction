...
# Create instances for file splitting and merging
input_dir = "./db/models/model_split_47"
output_file = "./model_sd_47.pth"

# Function to check and merge files if necessary
def check_and_merge_files(input_dir, output_file):
    if not os.path.exists(output_file):
        merge = Merge(inputdir=input_dir, outputdir=os.path.dirname(output_file), outputfilename=os.path.basename(output_file))
        merge.merge(cleanup=False)  # Set cleanup=True if you want to delete split files after merging

st.set_page_config(page_title='F1 Laps with ML', page_icon=None, layout='centered', initial_sidebar_state='auto')

db_dir = './db/'

# Check and merge files
check_and_merge_files(input_dir, output_file)
...
def get_times(year, _round, lap):
    if year <= 2020:
        # Load pre-cached race data for years <= 2020
        race_path = db_dir + f'/races_npy/{year}/{_round-1}_in.npy'
        race_out_path = db_dir + f'/races_npy/{year}/{_round-1}_exp.npy'
        if os.path.exists(race_path) and os.path.exists(race_out_path):
            race = np.load(race_path)
            race_out = np.load(race_out_path)
            if lap >= len(race):
            ...
            # Handle the case where the files don't exist (return empty arrays or handle the error)
            return [], []
    else:
        # Handle years > 2020 with API requests and caching
        cache_path = db_dir + f'/cache/{year}_{_round}_q.json'
        if not os.path.exists(cache_path):
            quali = requests.get(f'http://ergast.com/api/f1/{year}/{_round}/qualifying.json')
            if quali.status_code >= 200:
                j = quali.json()

                d_s_url = f'http://ergast.com/api/f1/{year-1}/driverStandings.json' if _round - 1 < 1 else f'http://ergast.com/api/f1/{year}/{_round-1}/driverStandings.json'
                c_s_url = f'http://ergast.com/api/f1/{year-1}/constructorStandings.json' if _round - 1 < 1 else f'http://ergast.com/api/f1/{year}/{_round-1}/constructorStandings.json'

                d_s = requests.get(d_s_url).json() if requests.get(d_s_url).status_code >= 200 else {}
                c_s = requests.get(c_s_url).json() if requests.get(c_s_url).status_code >= 200 else {}
        ...
        for i in range(20):
            if i < len(j['MRData']['RaceTable']['Races'][0]['QualifyingResults']):
                ...
                q_times = ['Q3', 'Q2', 'Q1']
                t = next((j['MRData']['RaceTable']['Races'][0]['QualifyingResults'][i].get(q) for q in q_times if q in j['MRData']['RaceTable']['Races'][0]['QualifyingResults'][i]), 0.0)
                t = time_to_int(t) or 0
...
def position_analysis(lap_in, out, num_of_laps=1, line_chart={}):
    ...
    for i in range(20):
        ...
        _tempcode = _ln[:3] if _code == '\\N' else _code
        ...
        _pitting = 'NA' if _pitting == 0 else _pitting
        _retired = _pos == 21
        _status = stat_unbed(_o[i*60 + 22: i*60 + 28], _retired)
        if _d_idx == 853:
            _status = 'Spun off'
        _time = 'NA' if _retired else f'{lt_unbed(_o[i*60 + 28:]):.1f}'
